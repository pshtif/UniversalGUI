...
namespace UniversalGUI
{
    public class GenericMenuItem
    {
        ...
        public GenericMenuItem(GUIContent p_content, bool p_separator, bool p_state, Action p_callback)
        {
            content = p_content;
        ...
        public GenericMenuItem(GUIContent p_content, bool p_separator, bool p_state, Action<object> p_callback, object p_data)
        {
            content = p_content;
            separator = p_separator;
            state = p_state;
    ...
    public class UniGUIGenericMenu
    {
        public List<GenericMenuItem> Items { get; private set; } = new List<GenericMenuItem>();
        
        public void AddItem(GUIContent p_content, bool p_state, Action p_callback)
        {
            Items.Add(new GenericMenuItem(p_content, false, p_state, p_callback));
        }

        public void AddItem(GUIContent p_content, bool p_state, Action<object> p_callback, object p_data)
        {
            Items.Add(new GenericMenuItem(p_content, false, p_state, p_callback, p_data));
        }

        public void AddSeparator(string p_path)
        {
            Items.Add(new GenericMenuItem(new GUIContent(p_path), true, false, null));   
        }
        
#if UNITY_EDITOR
        public void ShowAsEditorMenu()
        {
            UnityEditor.GenericMenu editorMenu = new UnityEditor.GenericMenu();
